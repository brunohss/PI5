...
class EmulatorHandler(BaseHTTPRequestHandler):
    ...
    def simulate_update(self, data):
        ...
        print("[SIMULATE] Atualizado:", data)
    ...
    # 🔥 carregar config do "SD"
        self.load_config(material)

        print("[INFO] Material →", material)

        self.send_json({"success": True, "material": material})
    # ================= SDCARD =================
    def save_config(self):
        material = self.state["material"]
        with open(f"config_{material}.json", "w") as f:
            json.dump(self.state["config"], f)

    def load_config(self, material):
        try:
            with open(f"config_{material}.json", "r") as f:
                self.state["config"] = json.load(f)
                print(f"[SD] Config carregada para {material}")
        except:
            self.state["config"] = self.materials[material]
            print(f"[SD] Usando padrão para {material}")
    ...
    def calculate_status(self, temp, hum, light, sound):
        ...
        if hum < c["humMin"] - HUM_TOL or hum > c["humMax"] + HUM_TOL:
            errors += 1
        if light > c["lightMax"] + LIGHT_TOL:
            errors += 1
        if sound > c["soundMaxDB"] + SOUND_TOL:
            errors += 1
    ...
    def send_index(self):
        ...
        <html>
        <head>
            <title>PrintSense Emulator V5.2</title>
            <style>
        ...
        <h1>🚀 PrintSense Emulator V5.2</h1>

        <h2>📡 Endpoints:</h2>
        <ul>
            <li>GET /api/data</li>
            <li>POST /api/material</li>
            <li>POST /api/simulate</li>
            <li>GET /api/logs</li>
            <li>GET /api/log/&lt;file&gt;</li>
            <li>GET /api/config</li>
            <li>POST /api/config</li>
            <li>GET /api/calibration</li>
            <li>POST /api/calibration</li>
        </ul>
...
def run():
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("Uso: python emulator.py [porta]")
            return

    print("\n" + "="*60)
    print("🤖 PrintSense Emulator V5.2 FINAL")
    print("="*60)
    print(f"✅ Rodando em: http://localhost:{port}")
    print(f"🌐 Rede: http://SEU_IP:{port}")
